		...
		<div class="boxB">
		  ...
		  $rec=$sql->fetch(PDO::FETCH_ASSOC);
			...
			$akiyoteDay=$rec['akiyoteDay'];
			 ...
			 ?>
			 <form method="post" action="estate_akiyote_moshikomi_che.php">
			 <p style="margin-top:70px;font-size:16px;"><?php echo '物件名【'.$real_name.'　'.$roomNumber.'号室】　住所：'.$address?></p>

			 <p><?php echo '家賃：'.number_format($yachin).'円　共益費：'.number_format($kyoeki).'円　敷金'.$shiki.'ヶ月　礼金'.$rei.'ヶ月　間取り：'.$madoriType; ?></p>
